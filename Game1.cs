...
namespace GME1003GoblinDanceParty
{
    public class Game1 : Game
    {
        ...
        protected override void Initialize()
        {
            ...
            _starAlpha = new List<float>();

            for (int i = 0; i < _numStars; i++)
            {
                float alpha = 0.25f + (float)_rng.NextDouble() * 0.75f;
                _starAlpha.Add(alpha);
            }
            ...
            for (int i = 0; i < _numStars; i++)
            {
                ...
                _starColors.Add(randomColor);
            }

            for (int i = 0; i < _numStars; i++)
            {
                float scale = 0.25f + (float)_rng.NextDouble() * 0.75f;
                _starScales.Add(scale);
            }
            for (int i = 0; i < _numStars; i++)
            {
                // 0f to 1f is a full rotation in MonoGame
                float rotation = (float)_rng.NextDouble();
                _starRotations.Add(rotation);
            }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            for (int i = 0; i < _numStars; i++) 
            {
                _spriteBatch.Draw(_starSprite,
                    ...
                    _starColors[i] * _starAlpha[i],     //set colour and transparency
                    _starRotations[i],
                    new Vector2(_starSprite.Width / 2, _starSprite.Height / 2), //ignore this
                    new Vector2(_starScales[i], _starScales[i]),
